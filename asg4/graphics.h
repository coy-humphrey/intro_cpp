...
class object {
   ...
   public:
      object (const shared_ptr<shape> &shape,
              const vertex &cent,
              const rgbcolor &col)
         : pshape (shape), center (cent), color (col) {}
      // Default copiers, movers, dtor all OK.
      void draw() { pshape->draw (center, color); }
      void draw_outline(const rgbcolor &col)
      {
         pshape->draw_outline (center, col);
      }
...
class window {
   ...
   public:
      ...
      static void main();
      static GLfloat move_speed;
      static GLfloat border_width;
      static rgbcolor border_col;
